...
#SBATCH --output=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/hiot-imagenet/sbatch/base_runtime.out # create a output file
#SBATCH --error=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/hiot-imagenet/sbatch/base_runtime.err # create a error file
...
OUTPUT_PATH="./local/base_runtime"


## OT LOSS uniform weight
CUDA_VISIBLE_DEVICES='0,1' python -m torch.distributed.launch --master_port 19735 --nproc_per_node=2 --use_env main.py \
--model deit_tiny_patch16_224 --batch-size 1024 --data-path $DATA_PATH --output_dir $OUTPUT_PATH \
--ot-loss --tree_folder_path ./tree --base-weight 1 --ot-weight 0
