...
#SBATCH --job-name=0.25ot# create a short name for your job
#SBATCH --output=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/full_weight/0.25ot.out # create a output file
#SBATCH --error=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/full_weight/0.25ot.err # create a error file
...
CUDA_VISIBLE_DEVICES='0,1' torchrun --nproc_per_node=2  --master_port=$MASTER_PORT deit/main_suppix_hier.py \
  --model cast_small \
  --batch-size 256 \
  --epochs 100 \
  ...
  --data-path ../dataset/ \
  --output_dir ./output/new/full_weight/0.25ot  \
  --ot_loss --ot_weight 0.25 \
