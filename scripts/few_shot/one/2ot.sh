...
#SBATCH --output=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/few_shot/one/2ot.out # create a output file
#SBATCH --error=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/few_shot/one/2ot.err # create a error file
...
torchrun --nproc_per_node=1  --master_port=12304 deit/main_suppix_hier.py \
  --model cast_small \
  --batch-size 256 \
  --epochs 100 \
  ...
  --data-path ../dataset/ \
  --output_dir ./output/few_shot/one/2ot \
  --ot_loss --ot_weight 2 \
  --base_weight 0.5 \
