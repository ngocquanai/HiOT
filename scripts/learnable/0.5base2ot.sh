...
#SBATCH --output=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results/learnable/0.5base2ot.out # create a output file
#SBATCH --error=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results/learnable/0.5base2ot.err # create a error file
...
torchrun --nproc_per_node=2  --master_port=29815 deit/main_suppix_hier.py \
  --model cast_small \
  --batch-size 256 \
  --epochs 100 \
  ...
  --data-path ../dataset/ \
  --output_dir ./output/learnable/0.5base2ot/ \
  --ot_loss --ot_weight 2 \
  --base_weight 0.5 \
  --learnable_ot \
