...
#SBATCH --output=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/eval/four/hcast.out # create a output file
#SBATCH --error=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/eval/four/hcast.err # create a error file
...
torchrun --nproc_per_node=1 --master_port=12328 deit/main_suppix_hier_eval.py \
  --model cast_small \
  --batch-size 256 \
  --epochs 100 \
  ...
  --data-path ../dataset/ \
  --output_dir ./output/eval/four/ \
  --globalkl --gk_weight 0.5 \
  --few_shot 4 \
  --finetune ./output/few_shot/four/checkpoint.pth --distributed
