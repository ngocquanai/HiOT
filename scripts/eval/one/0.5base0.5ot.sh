...
#SBATCH --output=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/eval/one/0.5weight_test.out # create a output file
#SBATCH --error=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/eval/one/0.5weight_test.err # create a error file
...
torchrun --nproc_per_node=1  --master_port=12311 deit/main_suppix_hier_eval.py \
  --model cast_small \
  --batch-size 256 \
  --epochs 100 \
  ...
  --data-path ../dataset/ \
  --output_dir ./output/eval/one/0.5base0.5ot_test \
  --ot_loss --ot_weight 0.5 \
  --base_weight 0.5 \
  --finetune ./output/few_shot/one/0.5base0.5ot/best_checkpoint.pth \
  --few_shot 1 \
