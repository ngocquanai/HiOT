...
#SBATCH --output=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/eval/one/hcast.out # create a output file
#SBATCH --error=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/eval/one/hcast.err # create a error file
...
torchrun --nproc_per_node=1 --master_port=12316 deit/main_suppix_hier_eval.py \
  --model cast_small \
  --batch-size 256 \
  --epochs 100 \
  ...
  --data-path ../dataset/ \
  --output_dir ./output/eval/one/ \
  --globalkl --gk_weight 0.5 \
  --finetune ./output/few_shot/one/checkpoint.pth --distributed
