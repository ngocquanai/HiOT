...
#SBATCH --output=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/eval/two/10ot.out # create a output file
#SBATCH --error=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results_new/eval/two/10ot.err # create a error file
...
torchrun --nproc_per_node=1  --master_port=12320 deit/main_suppix_hier_eval.py \
  --model cast_small \
  --batch-size 256 \
  --epochs 100 \
  ...
  --data-path ../dataset/ \
  --output_dir ./output/eval/two/10ot \
  --ot_loss --ot_weight 10 \
  --base_weight 0.5 \
  --finetune ./output/few_shot/two/10ot/best_checkpoint.pth \
  --few_shot 2 \
