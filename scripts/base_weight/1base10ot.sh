...
#SBATCH --job-name=10ot1base# create a short name for your job
#SBATCH --output=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results/base_weight/1base_10ot.out # create a output file
#SBATCH --error=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results/base_weight/1base_10ot.err # create a error file
...
torchrun --nproc_per_node=2  --master_port=19865 deit/main_suppix_hier.py \
  --model cast_small \
  --batch-size 256 \
  --epochs 100 \
  ...
  --data-path ../dataset/ \
  --output_dir ./output/base_weight/1base_10ot/ \
  --ot_loss --ot_weight 10 \
  --base_weight 1 \
