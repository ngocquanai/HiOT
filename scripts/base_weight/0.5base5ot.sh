...
#SBATCH --job-name=5ot0.5base# create a short name for your job
#SBATCH --output=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results/base_weight/0.5base_5ot.out # create a output file
#SBATCH --error=/lustre/scratch/client/movian/research/users/quanpn2/public/HiOT/results/base_weight/0.5base_5ot.err # create a error file
...
torchrun --nproc_per_node=4  --master_port=23865 deit/main_suppix_hier.py \
  --model cast_small \
  --batch-size 256 \
  --epochs 100 \
  ...
  --data-path ../dataset/ \
  --output_dir ./output/base_weight/0.5base5ot/ \
  --ot_loss --ot_weight 5 \
  --base_weight 0.5 \
